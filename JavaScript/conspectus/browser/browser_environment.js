...
// При defer или async скрипт будет грузиться в фоновом режиме, но
//      defer   отложит выполнение скрипта до полной готовности документа
//      async   запустит скрипт как можно раньше без блокир-я анализа док-а
// Когда есть оба атрибута, приоритет имеет async
// Асинхронные скрипты запускаются не по порядку расположения в документе
...
/// ЗАГРУЗКА СКРИПТОВ ПО ЗАПРОСУ
// Для загрузки скрипта по действию польз-ся использя import() (для модулей)
...
// Объявления "var" и "function" на верхн.ур-е созда.т св-ва в глобальном объекте
...
// Шаг 2. <script> без async, defar или type="module" добавл-ся в док. и запускается.
...
// Web-Worker - фоновый поток для выполнения задач без замораживания польз-го интерф-са
// Код в-воркера взаимод-ет с гланым потоком и другими в.в-ми через события асинх.сообщ.
...
// Источник опред-ся как протокол, хост и порт URL, откуда док. был загружен


/// МЕЖСАЙТОВЫЕ СКРИПТЫ     (cross-site scripting - XSS)
// - категория проблем безопасности, когда атакующий внедряет HTML-теги в целевой сайт
